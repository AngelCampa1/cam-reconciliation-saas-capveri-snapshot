...
import { getAllPersonas, getPersona } from "@/lib/content/pseo-data";
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${SITE_URL}/api/og?title=${encodeURIComponent(data.metaTitle)}&category=For%20Your%20Role`,
          width: 1200,
          height: 630,
          alt: data.metaTitle,
        },
...
export default async function PersonaPage({
  ...
  const articleSchema = {
    ...
    datePublished: data.datePublished,
    dateModified: data.dateModified,
    about: { "@type": "Occupation", name: data.role },
    url,
  };
          ...
          <article className="prose max-w-none">
            ...
            <section className="mb-10">
              ...
              <p className="text-muted-foreground mb-0">
                Want more?{" "}
                <Link
                  href={`/resources/roles/${data.slug}/cam-guide`}
                  className="text-primary font-medium hover:underline"
                >
                  Read the {data.name} CAM guide
                </Link>
                . It shows the errors and time you save in your role.
              </p>
            </section>
            ...
            {data.relatedTools.length > 0 && (
              <section className="mb-10">
                <RelatedContent
                  title="Free Tools for Your Role"
                  links={data.relatedTools.map((href) => ({
                    href,
