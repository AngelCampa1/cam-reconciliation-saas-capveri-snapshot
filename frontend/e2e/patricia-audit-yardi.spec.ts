/**
 * Patricia's Free Audit Flow — Yardi GL Upload
 *
 * True E2E — zero mocks. Runs against live Supabase + Cloudflare Worker backend.
 *
 * T1/T2: Upload flow tests (IngestionPage with real backend)
 * T3-T5: YoY comparison tests (real seeded GL data)
 */
...
test.describe("Patricia's Upload Flow — Yardi", () => {
  test.beforeEach(async () => {
    await cleanUploadBatches()
  })

  test('T1: Patricia uploads Yardi GL and sees source detected as Yardi', async ({
    ...
    await fileInput.setInputFiles(path.join(FIXTURE_DIR, 'yardi_gl_hou01_2024.csv'))

    // Real backend call — should show Yardi Voyager detected
    await expect(page.getByText(/Yardi Voyager/i)).toBeVisible({ timeout: 20000 })
    await expect(page.getByText(/Confidence:.*\d+%/i)).toBeVisible({ timeout: 5000 })
  })

  test('T2: 30 rows imported with 0 errors', async ({
    authenticatedPage: page,
  }) => {
    await page.goto('/ingestion')
    await page.waitForLoadState('networkidle')
...
test.describe("Patricia's YoY Analysis — Yardi", () => {
  test.beforeEach(async () => {
    // Ensure only the seeded baseline batches are present — any upload from T1/T2
    // would corrupt the YoY aggregation if left in the DB
    await cleanUploadBatches()
  })

  test('T3: YoY comparison reveals property tax gross-up — $30,003 overcharge', async ({
    authenticatedPage: page,
  }) => {
  ...
  test('T4: YoY comparison reveals HVAC CapEx misclassification — $10,800 overcharge', async ({
    authenticatedPage: page,
  }) => {
    await page.goto('/analysis/year-over-year')
    await page.waitForLoadState('networkidle')
    ...
    await expect(page.getByText('R&M - HVAC Repairs')).toBeVisible({ timeout: 15000 })
    await expect(page.getByText(/\+\$10,800/)).toBeVisible({ timeout: 10000 })
  })

  test('T5: YoY comparison shows controllable expenses above cap threshold', async ({
    authenticatedPage: page,
  }) => {
    await page.goto('/analysis/year-over-year')
    await page.waitForLoadState('networkidle')
    ...
    // Controllable Expenses pool should be visible with a positive variance
    await expect(page.getByText('Controllable Expenses')).toBeVisible({ timeout: 15000 })
    // +$12,980 variance (+9.16%) — check for positive dollar amount
