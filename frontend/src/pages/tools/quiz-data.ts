...
export const RISK_TIERS: { low: RiskTier; moderate: RiskTier; high: RiskTier } =
  {
    ...
    high: {
      label: 'High Risk',
      message: 'A tenant auditor would find issues within 30 days.',
...
export const VULNERABILITY_MAP: Record<string, string> = {
  ...
  'No independent reconciliation review':
    'Statements without a second review contain errors 3× more often.',
...
export function getTopVulnerabilities(answers: number[]): string[] {
  ...
  const allLabels = Object.keys(VULNERABILITY_MAP)
  let padIdx = 0
  while (result.length < 3 && padIdx < allLabels.length) {
    const label = allLabels[padIdx]
    if (label !== undefined && !seen.has(label)) result.push(label)
    padIdx++
  }
