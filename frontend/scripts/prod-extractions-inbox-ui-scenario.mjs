...
const outputDir = resolve(
  repoRoot,
  'e2e-adhoc',
  `prod-extractions-inbox-ui-${runId}`
)
...
async function runScenario() {
  ...
  const propertyName = `[PROD-TEST] Extraction Inbox Tower ${suffix}`
  const unitNumber = `Inbox-${suffix.toUpperCase()}`
  const tenantName = `[PROD-TEST] Extraction Inbox Tenant ${suffix}`
  const documentName = `prod-test-extractions-inbox-${suffix}.pdf`
  const created = {
    propertyId: null,
    unitId: null,
    leaseId: null,
  ...
  try {
    const property = await expectJson('/api/v1/properties', {
      ...
      body: {
        name: propertyName,
        address_line1: '910 Prod Extraction Inbox Way',
        city: 'Austin',
        state: 'TX',
        postal_code: '78710',
    ...
    const uploadedDocument = await uploadDocument({
      ...
      pdfBytes: minimalPdfBytes({
        title: 'CapVeri extraction inbox fixture',
        tenantName,
      }),
    })
    ...
    check(
      'document upload creates pending extraction inbox fixture',
      {
    ...
    created.documentStorageKey = detail.storage_key
    report.generated.documentStorageKey = detail.storage_key
    check(
      'api confirms generated extraction is pending before browser sweep',
      {
        id: detail.id,
        filename: detail.filename,
        status: detail.status,
        property_id: detail.property_id,
        lease_id: detail.lease_id,
      },
      {
    ...
    await runBrowserScenario({ documentName, documentId: detail.id })
  } finally {
    await cleanup(created)
  }
}

async function runBrowserScenario({ documentName, documentId }) {
  ...
  try {
    ...
    const page = await newTrackedPage(context, 'extractions-inbox')
    try {
      await gotoAndVerifyAllStatuses(page, documentName, documentId)
      await selectStatus(page, 'Pending')
      await verifyPendingFilter(page, documentName, documentId)
      await selectStatus(page, 'Ready for Review')
      await verifyReadyForReviewFilter(page, documentName)
    } finally {
      await page.close()
    }
  } finally {
...
async function gotoAndVerifyAllStatuses(page, documentName, documentId) {
  const response = await page.goto(`${appUrl}/extractions`, {
    waitUntil: 'networkidle',
    timeout: 60_000,
  })
  await page.waitForTimeout(1000)
  await expectVisible(page, 'Document Extractions')
  await expectVisible(
    page,
    'Review and confirm the lease data pulled from each PDF'
  )
  await expectVisible(page, documentName)
  await expectVisible(page, 'Pending')
  await expectVisible(page, 'Process')
  await page.screenshot({
    path: resolve(outputDir, 'extractions-inbox-all-statuses.png'),
  })
  report.browser.screenshots.push('extractions-inbox-all-statuses.png')
  check(
    'browser all-status extraction inbox shows generated pending document',
    {
      status: response?.status() ?? null,
      final_url: page.url(),
      document_visible: await visibleText(page, documentName),
      process_action_visible: await visibleText(page, 'Process'),
      document_id_not_in_url: !page.url().includes(documentId),
    },
    {
      status: 200,
      ...
      document_id_not_in_url: true,
    }
  )
}

async function verifyPendingFilter(page, documentName, documentId) {
  await expectVisible(page, documentName)
  await expectVisible(page, 'Pending')
  await page.screenshot({
    path: resolve(outputDir, 'extractions-inbox-pending-filter.png'),
  })
  report.browser.screenshots.push('extractions-inbox-pending-filter.png')
  check(
    'browser pending filter keeps generated pending document visible',
    {
      document_visible: await visibleText(page, documentName),
      process_action_visible: await visibleText(page, 'Process'),
      document_id_not_in_url: !page.url().includes(documentId),
      saw_pending_api_request: report.browser.requests.some(
        (request) =>
    ...
    },
    {
      ...
      process_action_visible: true,
      document_id_not_in_url: true,
      saw_pending_api_request: true,
    }
  )
}

async function verifyReadyForReviewFilter(page, documentName) {
  await page.screenshot({
    path: resolve(outputDir, 'extractions-inbox-ready-filter.png'),
  })
  report.browser.screenshots.push('extractions-inbox-ready-filter.png')
  check(
    'browser ready-for-review filter excludes generated pending document',
    {
      generated_document_hidden: !(await visibleText(page, documentName)),
      empty_state_visible: await visibleText(
        page,
        'No extractions with this status'
      ),
      saw_ready_api_request: report.browser.requests.some(
        (request) =>
          request.url.includes('/api/v1/extractions') &&
          request.url.includes('status=ready_for_review')
      ),
    },
    {
      generated_document_hidden: true,
      empty_state_visible: await visibleText(
        page,
        'No extractions with this status'
      ),
      saw_ready_api_request: true,
    }
  )
}

async function selectStatus(page, optionName) {
