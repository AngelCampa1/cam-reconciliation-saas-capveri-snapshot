...
const outputDir = resolve(
  repoRoot,
  'e2e-adhoc',
  `prod-export-public-token-negative-${runId}`
)
...
async function runScenario() {
  ...
  const propertyName = `[PROD-TEST] Export Token Tower ${suffix}`
  const unitNumber = `Token-${suffix.toUpperCase()}`
  const tenantName = `[PROD-TEST] Export Token Tenant ${suffix}`
  const poolName = `[PROD-TEST] Export Token Pool ${suffix}`
  const fileName = `yardi-export-token-prod-${suffix}.csv`
  ...
  try {
    const property = await expectJson('/api/v1/properties', {
      ...
      body: {
        name: propertyName,
        address_line1: '920 Prod Export Token Way',
        city: 'Austin',
        state: 'TX',
        postal_code: '78705',
    ...
    created.publicDownloadUrl = redownload.download_url
    report.generated.redactedPublicDownloadUrl = redactSensitiveUrl(
      redownload.download_url
    )
    check(
      'public token download returns persisted PDF bytes',
      {
        ...
        same_sha256: publicFile.sha256 === pdf.sha256,
      },
      {
        status: 200,
        content_type: 'application/pdf',
    ...
    const missingToken = await expectAbsoluteStatus(
      `${apiUrl}/api/v1/export/download/file`,
      { status: 400 }
    )
    check(
      'public export file route rejects missing token',
      {
        status: missingToken.status,
        code: missingToken.json?.error?.code,
      },
      {
        status: 400,
        code: 'missing_token',
    ...
    const tamperedTokenUrl = tamperExportToken(redownload.download_url)
    const tampered = await expectAbsoluteStatus(tamperedTokenUrl, {
      status: 400,
    })
    check(
      'public export file route rejects tampered token',
      {
        status: tampered.status,
        code: tampered.json?.error?.code,
      },
      {
        status: 400,
...
async function cleanup(created, period) {
  ...
  if (created.exportHistoryId) {
    ...
    if (created.publicDownloadUrl) {
      await attemptCleanup(
        exportFailures,
        'verify old public token URL no longer returns bytes',
        () => expectDeletedPublicTokenUrl(created.publicDownloadUrl)
      )
    }
...
async function expectAbsoluteStatus(url, options) {
  ...
  if (response.status !== options.status) {
    throw new Error(
      `GET ${redactSensitiveUrl(url)} returned ${response.status}, expected ${options.status}: ${text.slice(0, 500)}`
    )
  }
  return { status: response.status, json, text }
}

async function expectDeletedPublicTokenUrl(url) {
  const result = await expectAbsoluteStatus(url, { status: 404 })
  const ok = result.json?.error?.code === 'report_not_found'
  report.cleanup.push({
    path: redactSensitiveUrl(url),
  ...
  if (!ok) {
    throw new Error(
      `Deleted public token URL returned unexpected response: ${result.text.slice(0, 500)}`
    )
  }
}
...
function tamperExportToken(value) {
  const parsed = new URL(value)
  const tokenValue = parsed.searchParams.get('token')
  if (!tokenValue || tokenValue.length < 2) {
    throw new Error(
      `Expected token-bearing export URL: ${redactSensitiveUrl(value)}`
    )
  }
  const last = tokenValue.at(-1)
  const replacement = last === 'a' ? 'b' : 'a'
  parsed.searchParams.set('token', `${tokenValue.slice(0, -1)}${replacement}`)
