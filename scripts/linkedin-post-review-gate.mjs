...
function postsFromPath(path) {
  ...
  if (
    name === "readme.md" ||
    name === "strategy.md" ||
    name === "verify_findings.md" ||
    name === "qa_report.json" ||
    name === "postiz_bulk_state.json" ||
    /^batch_[a-z]\.json$/i.test(name) ||
    /^review2?_batch_/.test(name)
  ) {
    return [];
  }
  if (ext === ".jsonl") {
  ...
  if (ext === ".md" && !text.trim().includes("scheduled_date:")) return [];
  return [
